...
bool valid(game_state s, int r, int c)
{
	...
	if (r <= s->row_size && c <= s->col_size) //if variable r and c are greater than maximum column width and row size
	{
		return true;
	}
	else
...
bool taken(game_state s, int r, int c)
{
	...
	//check this position is be taken or not. should minus 1 to take the real position number 
	if (s->board[r - 1][c - 1] != 0)
	{
		return true;
	}
	else
...
bool wall(game_state s, int r, int c)
{
	...
	// check this is a wall or not, minus 1 to get the real number. 
	if (s->board[r - 1][c - 1] == WALL)
	{
		return true;
	}
	else
...
bool move(game_state s, int r, int c)
{
	...
	//check the positions the program wants to execute are taken or a wall
	if (!wall(s, r, c) && !taken(s, r, c))
	{

		set_current_column(s, c);						// set current column for the game state s
		set_current_row(s, r);						// set current row for the game state s
		set_visited(s->board[r - 1][c - 1], true);  //update the position
...
bool solved(game_state s)
{
	...
	// check the maze has been solved or not. we need minus 1 to get the real world values
	if (s->board[get_current_row(s) - 1][get_current_column(s) - 1] == s->goal)
	{
		return true;
	}
	else
