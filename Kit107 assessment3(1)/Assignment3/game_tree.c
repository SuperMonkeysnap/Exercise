...
void generate_levelBF(game_tree t, queue q)
{
	...
	for (int i = 0; i < MOVE_COUNT; i++)
	{
		...
		if (valid(s, x, y) && !wall(s, x, y) && !taken(s, x, y))
		{
			...
			n = clone(s);			// game state n = gamestate s
			move(n, x, y);			// move copied gamestate with the new moves
			set_data(c, n);			// set the new tree data from new gamestate
			add(q, n);				// let the gamestate added to the queue
			set_parent(n, t);		// n becomes parented to t

			//change the data in each t'leaf, from left and right. 
...
game_tree build_gameBF(game_tree t, queue q)
{
	...
	if (!is_empty_queue(q))
	{
		rear(q);				//Remove head of the queue
		generate_levelBF(t, q); //Generate the next level of the tree 
	}
	// COMPLETE ME!!!
